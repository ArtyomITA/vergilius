...
# Percorsi ricavati dalla posizione dello script (scripts\ sta dentro il progetto):
# niente cartella fissa, il progetto funziona ovunque sia clonato.
$radice = Split-Path -Parent $PSScriptRoot
$logs = Join-Path $radice 'logs'
...
if (Gia-Su 7001) {
    ...
    $argomenti = @((Join-Path $radice 'boot\vergilius_boot.py'), '--open')
    ...
    if (-not $py) { $py = Join-Path $radice 'odysseus\venv\Scripts\python.exe' }
